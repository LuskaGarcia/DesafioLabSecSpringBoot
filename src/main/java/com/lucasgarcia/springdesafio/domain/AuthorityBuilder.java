...
public class AuthorityBuilder implements Serializable {
	...
	public static Authority generateAuthority(boolean isRoot, String name, Authority issuer,
		...
		if (issuer == null) {
			...
			rootCertIssuer = new X500Name(name); // o x500name é uma classe entidade que tem suporte aos atributos do
													// x500
			rootCertSubject = rootCertIssuer; // aqui define que é um AC auto assinado, pois diz que o issuer (emissor)
												// é o mesmo que o subject (quem quer o certificado)
			...
			issuedSerialNum = new BigInteger(Long.toString(new SecureRandom().nextLong())); // armazena o serialnumber
																							// que é um grande inteiro

			// Issued By and Issued To same for root certificate
			issuedCertSubject = new X500Name(name); // o x500name é uma classe entidade que tem suporte aos atributos do
													// x500
